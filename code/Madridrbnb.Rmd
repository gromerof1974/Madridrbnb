...
Se observa que el coeficiente de correlación entre ambas variables es muy bajo (0.20) 
...
Se seleccionan las variables que vamos a  incluir en el modelo y comprobamos su correlación:
...
La correlación más alta se da entre **price_trans** y **longitude**, pero es muy baja. No parece tener sentido intentar generar un modelo de regresión lineal que permita predecir el precio en función de la variable longitud.

Se crean variables dummy para las variables categóricas que incorporamos al modelo: **room_type**, **neighbourhood_group** y **neighbourhood**.
...
# Unimos variables dummy y eliminamos las categóricas
...
La bondad del ajuste se mide con el parámetro R-squared que es de 0.46. Cuando este valor es próximo a 1 es indicativo de un buen modelo de ajuste. Pero no es el caso de este modelo.
...
* Residuals vs Fitted: se aprecia la aleatoriedad de los resultados y como la varianza es constante a lo largo del eje X.
...
```{r}
# Se crea la predicción de los datos de entrenamiento
...
Según las [estadísticas del Ayuntamiento de Madrid](http://www-2.munimadrid.es/CSE6/control/mostrarDatos) la superficie media de la vivienda en Madrid es de 82 m2. 
